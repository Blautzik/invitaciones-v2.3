...
const Header = ({ coverImage, title, date, coverImagePc }) => {
    ...
    return (
        <div className="h-[100dvh] bg-slate-800 relative overflow-hidden">
          <div className="top-0 w-full h-full relative">
            ...
            <div className='absolute top-0 left-0 w-full h-full flex flex-col justify-between items-center'>
              ...
              <div className='flex flex-col items-center'>
                <h2 className={`${openSans.className} text-gray-50 max-w-xs text-center text-xl tracking-widest font-[100]`}>Vi gifter os om</h2>
                <Countdown date={date} />
                <Link href='#info' scroll={false}>
                  <button className={`${openSans.className} py-3 px-9 bg-white rounded-full text-gray-900 font-[900] text-center mb-16`}>MERE INFO</button>
                </Link>
              </div>
            </div>
          </div>
